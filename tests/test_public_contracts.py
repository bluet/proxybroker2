...
from proxybroker.errors import (
    ProxyError, NoProxyError, ResolveError, ProxyConnError, 
    ProxyRecvError, ProxySendError, ProxyTimeoutError, ProxyEmptyRecvError
)
...
class TestBrokerPublicContract:
    ...
    def test_broker_init_signature(self):
        ...
        expected_params = {
            'self', 'queue', 'timeout', 'max_conn', 'max_tries', 
            'judges', 'providers', 'verify_ssl', 'loop', 'stop_broker_on_sigint', 'kwargs'
        }
        actual_params = set(sig.parameters.keys())
        assert expected_params == actual_params, f"Missing params: {expected_params - actual_params}"
    ...
    async def test_broker_find_signature(self):
        ...
        expected_params = {
            'self', 'types', 'data', 'countries', 'post', 'strict', 'dnsbl', 'limit', 'kwargs'
        }
        actual_params = set(sig.parameters.keys())
    ...
    async def test_broker_grab_signature(self):
        ...
        expected_params = {
            'countries', 'post', 'strict', 'dnsbl', 'limit'
        }
        actual_params = set(sig.parameters.keys())
        assert expected_params == actual_params
    ...
    async def test_broker_serve_signature(self):
        ...
        required_params = {
            'host', 'port', 'types', 'countries', 'post', 'strict', 'dnsbl',
            'limit', 'lvl', 'prefer_connect', 'min_req_proxy', 'max_error_rate',
            'max_resp_time', 'http_allowed_codes', 'backlog'
        }
        actual_params = set(sig.parameters.keys())
...
class TestProxyPublicContract:
    ...
    def test_proxy_init_signature(self):
        ...
        expected_params = {
            'self', 'host', 'port', 'types', 'timeout', 'verify_ssl'
        }
        actual_params = set(sig.parameters.keys())
        assert expected_params == actual_params
...
class TestProxyPoolPublicContract:
    ...
    def test_proxy_pool_init_signature(self):
        ...
        expected_params = {
            'self', 'proxies', 'min_req_proxy', 'max_error_rate', 
            'max_resp_time', 'min_queue', 'strategy'
        }
        actual_params = set(sig.parameters.keys())
        assert expected_params == actual_params
...
class TestServerPublicContract:
    ...
    def test_server_init_signature(self):
        ...
        expected_params = {
            'self', 'host', 'port', 'proxies', 'timeout', 'backlog', 
            'http_allowed_codes', 'prefer_connect'
        }
        actual_params = set(sig.parameters.keys())
        assert expected_params == actual_params
...
class TestErrorContractStability:
    ...
    def test_exception_hierarchy(self):
        ...
        proxy_errors = [
            ProxyConnError, ProxyRecvError, ProxySendError, 
            ProxyTimeoutError, ProxyEmptyRecvError
        ]
...
class TestExportedInterfaceStability:
    ...
    def test_main_module_exports(self):
        ...
        required_exports = [
            'Broker', 'Proxy', 'ProxyPool', 'Server', 
            'Checker', 'Judge', 'Provider'
        ]
    ...
    def test_error_module_exports(self):
        ...
        required_errors = [
            'ProxyError', 'NoProxyError', 'ResolveError',
            'ProxyConnError', 'ProxyRecvError', 'ProxySendError',
            'ProxyTimeoutError', 'ProxyEmptyRecvError'
        ]
